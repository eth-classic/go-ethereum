...
setup() {
	...
	if [ "$BATS_TEST_NUMBER" -eq 1 ]; then
		...
		curl -L -o "$TMP_DIR/gethc3.3.zip" https://github.com/ethereumclassic/go-ethereum/releases/download/v3.3.0/geth-classic-"$TEST_OS_C"-v3.3.0-1-gdd95f05.zip
...
@test "shouldnot migrate datadir /Ethereum/ -> /EthereumClassic/ from ETC3.3 schema | --chain kitty (invalid config)" {
	...
	[ -d "$DATA_DIR_PARENT"/"$DATA_DIR_NAME_EX"/chaindata ]
		...
		# Set up custom net config.
	mkdir -p $DATA_DIR_PARENT/$DATA_DIR_NAME/kitty
	cp "$GOPATH/src/github.com/ethereumclassic/go-ethereum/cmd/geth/testdata/chain_config_dump-invalid-coinbase.json" $DATA_DIR_PARENT/$DATA_DIR_NAME/kitty/chain.json
	sed -i.bak s/mainnet/kitty/ $DATA_DIR_PARENT/$DATA_DIR_NAME/kitty/chain.json
